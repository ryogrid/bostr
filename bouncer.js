  ...
  relay.on('message', data => {
    ...
    switch (data[0]) {
      case "EVENT": {
        ...
        if ('since' in client.subs.get(data[1]) && client.subs.get(data[1]).since > data[2].created_at) return;
        if ('until' in client.subs.get(data[1]) && data[2].created_at > client.subs.get(data[1]).until) return;
